...
public class EditarServico extends javax.swing.JFrame {
    ...
    public EditarServico(int id, String perfil, ModeloServico mode, String caminho) {
        initComponents();
        ...
        this.idServico = mode.getIdservico();

        liga.conexao();
        liga.executeSql("select * from servico WHERE idservico = " + this.idServico);

        try {
            if (liga.rs.first()) {
                this.jTextField_descricao.setText(liga.rs.getString("descricao"));
                this.jTextField_custo.setText(liga.rs.getString("custo"));
                this.jTextField_preco.setText(liga.rs.getString("preco"));
                this.jTextField_nomeDaFoto.setText(liga.rs.getString("nome_da_foto"));
                this.revert(liga.rs.getBytes("foto"));
                this.jTextAreaOBS.setText(liga.rs.getString("observacao"));
                ...
                jCheckBox_retencao.setSelected(liga.rs.getBoolean("retencao"));
                liga.deconecta();
            }
        } catch (SQLException ex) {
...
//                preco = preco_unit + (preco_unit * 0.14);
            ...
            PreparedStatement pst = liga.con.prepareStatement("UPDATE servico  SET descricao=?, imposto_iva=?, motivo=?, "
                    + "observacao=?, custo=?, preco=?, nome_da_foto=?, foto=?, preco_unitario=?, lucro=?, "
                    + "margem_de_lucro=?, valor_iva=?,retencao=? "
