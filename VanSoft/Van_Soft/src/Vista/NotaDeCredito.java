 ...
 * @author Gonga
 */
public class NotaDeCredito extends javax.swing.JFrame {
    ...
    public NotaDeCredito(int id, String perfil, String CAMINHO) {
        try {
            initComponents();
    ...
    private void initComponents() {
        ...
        jListDoc = new javax.swing.JList<String>();
        ...
        jListOrdem = new javax.swing.JList<String>();
        ...
        buttonGroupOrigem.add(jRadioButton1);
        jRadioButton1.setSelected(true);
        jRadioButton1.setText("Fatura recibo");
        ...
        buttonGroupOrigem.add(jRadioButton2);
        jRadioButton2.setText("Fatura");
        ...
        jListDoc.setModel(new javax.swing.AbstractListModel() {
            String[] strings = { "Item 1", "Item 2", "Item 3", "Item 4", "Item 5" };
            public int getSize() { return strings.length; }
            public Object getElementAt(int i) { return strings[i]; }
        });
        ...
        jListCliente.setModel(new javax.swing.AbstractListModel() {
            String[] strings = { "Item 1", "Item 2", "Item 3", "Item 4", "Item 5" };
            public int getSize() { return strings.length; }
            public Object getElementAt(int i) { return strings[i]; }
        });
        ...
        jListOrdem.setModel(new javax.swing.AbstractListModel() {
            String[] strings = { "Item 1", "Item 2", "Item 3", "Item 4", "Item 5" };
            public int getSize() { return strings.length; }
            public Object getElementAt(int i) { return strings[i]; }
        });
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (selected != -1) {
            if (jTextField_objectivo.getText().length() > 9) {
                String origem = jList5.getSelectedValue();
                if (JOptionPane.YES_OPTION == JOptionPane.showConfirmDialog(rootPane, "Deseja emitir a nota de crédito referente a fatura número " + origem + " " + jList4.getSelectedValue() + "?", null, JOptionPane.YES_NO_OPTION)) {
                    CreateNota(jList4.getSelectedValue());
                    int lastNota = this.liga.UltimoId("nota_de_credito", "idnota_de_credito", this.id, this.id_turno);

                    try {
                        //this.CreateLiquidacao();
                        liga.conexao();
                        PreparedStatement pst = liga.con.prepareStatement("UPDATE turno set nota_de_credito = nota_de_credito + " + jTextField_valor.getText() + ", "
                    ...
                    ME.setIndicativo("NC " + ME.getIndicativo());
                    String NUM = " 0";
                    if (lastNota > 9) {
                   ...
                   String nome_do_ficheiro  = Aux;
                    ...
                    if (chooser.showSaveDialog(null) == JFileChooser.APPROVE_OPTION) {
                        ...
                        try {
                            ...
                            e.setME(ME);
                            // ModeloUtilizador MU = null; MF
                            e.setMU(MU);
                            e.setMC(MC);
                            e.setMF(new ModeloFatura(lastNota));
                            e.exportA42_NotaCredito("Nota de credito", Aux, jList5.getSelectedValue() + " " + jList4.getSelectedValue(), Double.parseDouble(this.jTextField_valor.getText()), jTextField_objectivo.getText());
                ...
                jTextField_objectivo.requestFocus();
                JOptionPane.showMessageDialog(null, "Deves escrever um motivo de pelo menos 10 caracteres.", "Aviso", JOptionPane.WARNING_MESSAGE);
            }
        } else {
            JOptionPane.showMessageDialog(rootPane, "A nota de crédito é um documento rectificativo que serve para anular o valor de uma factura. \nTem que estar associado à uma factura obrigatoriamente.", "Nota não emitida", JOptionPane.WARNING_MESSAGE);
        }
    ...
    private void jList4MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jList4MouseClicked
        ...
        if (selected != -1) {
            ...
            jList5.setSelectedIndex(selected);
            String Ident = jList4.getSelectedValue();
            //System.out.println(Ident);
            liga.conexao();
            if (this.jRadioButton1.isSelected()) {
                liga.executeSql("select idfactura, total, idcliente from fatura "
                        + "WHERE idfactura = " + Ident + " Order by idfactura");
            } else {
                liga.executeSql("select idfatura_recibo, total, idcliente from recibo "
                        + "WHERE idfatura_recibo = " + Ident + " Order by idfatura_recibo");
            }
...
//            System.out.println("hhhhhh");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(NotaDeCredito.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new NotaDeCredito().setVisible(true);
            }
        });
    }
    ...
    public boolean CreateNota(String idfactura) {
        ...
        try {
            ...
            PreparedStatement pst = liga.con.prepareStatement("Insert into nota_de_credito (idcliente,data,valor,observacao,objectivo,idfactura"
                    + ",indicativo,idutilizador, idturno,origem,serie)  "
                    + "values (?,?,?,?,?,?,'" + this.ME.getIndicativo() + "'," + this.id + "," + this.id_turno + ",?,?)");
            ...
            JOptionPane.showMessageDialog(null, "\n" + ex, "Dados não inseridos", JOptionPane.WARNING_MESSAGE);
        }
        liga.deconecta();
        return false;
    }
...
//    }
    private void BuscarFacturasT(String Ident) {

        liga.conexao();
        liga.executeSql("select idfactura,indicativo from fatura WHERE  idcliente = " + Ident + " AND idfactura NOT IN (select idfactura from nota_de_credito WHERE origem!=1) Order by idfactura");
        ...
        try {
            while (liga.rs.next()) {
                ...
                if (this.jRadioButton1.isSelected()) {
                    model_List5.addElement("FR " + (liga.rs.getString(2)));
                } else {
                    model_List5.addElement("FT " + (liga.rs.getString(2)));
                }
    ...
    private void BuscarFacturas(String Ident) {
        ...
        liga.conexao();
        liga.executeSql("select idfatura_recibo,indicativo from recibo WHERE idcliente = " + Ident + " AND idfatura_recibo NOT IN (select idfactura from nota_de_credito WHERE origem=1) Order by idfatura_recibo");
        model_List4.removeAllElements();
        model_List5.removeAllElements();
        try {
            while (liga.rs.next()) {
